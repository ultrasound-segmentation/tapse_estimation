...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train U-Net model for keypoint detection."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_keypoints", type=int, default=3, help="Early stopping patience"
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="U-Net",
        help='name of the model: supported "U-Net"',
    )
    parser.add_argument(
        "--save_images",
        action="store_true",
        help="If to save test images with predictions",
    )
    parser.add_argument(
        "--train_data",
        type=str,
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="dl_mapse/Data/best_loss_weights_unet_light.pth",
    ...
    parser.add_argument(
        "--loss",
        type=str,
        default="ordered_distance",
        help='select the type of loss: "MSE" or "distance"',
    )
    parser.add_argument(
        "--wandb_entity", type=str, default=None, help="master_thesis_NTNU_mmissana"
    )
    parser.add_argument(
        "--save_model_path", type=str, default=None, help="Path to save trained model"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.9,
        help="Treshold for center of mass calculation",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Number of filters in the first layer of the model when using monai unet",
    )
    parser.add_argument(
        "--depth",
        type=int,
        default=6,
        help="Number of layers in the model when using monai unet",
    )
    parser.add_argument(
        "--dropout",
        type=float,
        default=0.0,
        help="Dropout rate for the model, supported only for monai unet",
    )
    parser.add_argument(
        "--n_residuals",
        type=int,
        default=0,
        help="Number of residual blocks in the model when using monai unet",
    )
...
def train_model(
    ...
    early_stopping = EarlyStopping(
        monitor="val_loss", mode="min", patience=patience, path=checkpoint_path
    )
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(
            ...
            for images, masks in train_loader:
                ...
                if (
                    "ResNet" not in args.model and "resnext" not in args.model
                ):  # If not using ResNet, compute center of mass
        ...
        wandb.log(
            {
                "train_loss": avg_loss,
                "val_loss": val_loss,
                "learning_rate": scheduler.currentlr,
            }
        )
        ...
        if early_stopping.early_stop:
            ...
            model.load_state_dict(
                torch.load(
                    os.path.join(checkpoint_path, "best_model.pth"), map_location=device
                )["model_state_dict"]
            )
            # save best model
                ...
                com_tensor = torch.stack(
                    [
                        torch.stack(
                            [
                                center_of_mass(
                                    mask, device=device, normalize=False, thresh=thresh
                                )
                                for mask in output
                            ]
                        )
...
def main():
    ...
    train_dataset = KeypointDataset(
        args.train_data, filter=True, model_type=args.model, transform=args.augm_version
    )
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=args.batch_size, shuffle=False, generator=g
    )
    ...
    if args.model == "monai_U-Net":
        model = monai_UNet(
            ...
            num_residuals=args.n_residuals,
        ).to(device)
